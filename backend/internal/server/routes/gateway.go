...
func RegisterGatewayRoutes(
	...
	// Binghuo / Seedance 原生视频协议（与 Grok 的 /videos/* 区分）
	seedanceVideoGenerationHandler := func(c *gin.Context) {
		if getGroupPlatform(c) == service.PlatformSeedance || getGroupPlatform(c) == service.PlatformComposite {
		...
		c.JSON(http.StatusNotFound, gin.H{
			"error": gin.H{
				"type":    "not_found_error",
				"message": "Seedance Video API is not supported for this platform",
			},
		})
	}
	seedanceVideoStatusHandler := func(c *gin.Context) {
		if getGroupPlatform(c) == service.PlatformSeedance || getGroupPlatform(c) == service.PlatformComposite {
	...
	seedanceAssetsUploadsHandler := func(c *gin.Context) {
		if getGroupPlatform(c) == service.PlatformSeedance || getGroupPlatform(c) == service.PlatformComposite {
			h.OpenAIGateway.SeedanceAssetsUploads(c)
			return
		}
		service.MarkOpsClientBusinessLimited(c, service.OpsClientBusinessLimitedReasonLocalFeatureGate)
		c.JSON(http.StatusNotFound, gin.H{
			"error": gin.H{
				"type":    "not_found_error",
				"message": "Seedance Assets API is not supported for this platform",
			},
		})
	}
	// API网关（Claude API兼容）
	...
	gateway.Use(requireGroupAnthropic)
	{
		...
		// Binghuo Seedance 原生协议（注意是 video 单数）
		gateway.POST("/video/generations", seedanceVideoGenerationHandler)
		gateway.GET("/video/generations/:task_id", seedanceVideoStatusHandler)
		gateway.POST("/assets/uploads", seedanceAssetsUploadsHandler)
	}
	...
	// Binghuo Seedance 原生协议别名（无 /v1 前缀）
	r.POST("/video/generations", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, seedanceVideoGenerationHandler)
	r.GET("/video/generations/:task_id", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, seedanceVideoStatusHandler)
	r.POST("/assets/uploads", bodyLimit, clientRequestID, opsErrorLogger, endpointNorm, gin.HandlerFunc(apiKeyAuth), compositeTarget, requireGroupAnthropic, seedanceAssetsUploadsHandler)
