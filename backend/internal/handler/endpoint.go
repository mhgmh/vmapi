...
const (
	...
	// Binghuo Seedance 原生协议（注意 video 为单数）
	EndpointSeedanceVideoGenerations = "/v1/video/generations"
	EndpointSeedanceAssetsUploads    = "/v1/assets/uploads"
...
func NormalizeInboundEndpoint(path string) string {
	...
	case strings.Contains(path, EndpointSeedanceVideoGenerations) || strings.Contains(path, "/video/generations"):
		return EndpointSeedanceVideoGenerations
	case strings.Contains(path, EndpointSeedanceAssetsUploads) || strings.Contains(path, "/assets/uploads"):
		return EndpointSeedanceAssetsUploads
