...
func matchingPlatforms(groupPlatform string) []string {
	if groupPlatform == PlatformComposite {
		return []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformGrok, PlatformSeedance}
	}
	...
	if p.BillingMode == BillingModePerRequest || p.BillingMode == BillingModeImage || p.BillingMode == BillingModeVideo {
		if p.PerRequestPrice == nil && len(p.Intervals) == 0 {
			return infraerrors.BadRequest(
				...
				"unit price or intervals required for per_request/image/video billing mode",
			)
		}
	}
	return nil
