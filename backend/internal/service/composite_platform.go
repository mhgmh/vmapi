...
func DetectModelPlatform(model string) (string, bool) {
	...
	case normalized == "grok" || strings.HasPrefix(normalized, "grok-"):
		return PlatformGrok, true
	case isSeedanceModelID(model) || isSeedanceModelID(normalized):
		return PlatformSeedance, true
	default:
		return "", false
	}
...
// isSeedanceModelID reports whether model matches the Binghuo Seedance catalog.
// Catalog lookup is case-sensitive (e.g. SD2.0-720P); lowercased fallback covers
// common lowercase aliases that still resolve via LookupSeedanceModel when present.
func isSeedanceModelID(model string) bool {
	trimmed := strings.TrimSpace(model)
	if trimmed == "" {
		return false
	}
	if _, ok := LookupSeedanceModel(trimmed); ok {
		return true
	}
	// Case-insensitive scan for composite detector only (catalog itself stays case-sensitive).
	lower := strings.ToLower(trimmed)
	for id := range seedanceModelByID {
