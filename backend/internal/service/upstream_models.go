...
// buildSeedanceUpstreamModelsRequest probes Binghuo/Seedance OpenAI-compatible /v1/models.
// Binghuo exposes a standard models list endpoint (Bearer auth) at the configured base_url.
func (s *AccountTestService) buildSeedanceUpstreamModelsRequest(ctx context.Context, account *Account) (*http.Request, error) {
	if account == nil {
		return nil, newUpstreamModelSyncConfigError("Account is required", nil)
	}
	if !account.IsSeedanceAPIKey() {
		return nil, newUpstreamModelSyncUnsupportedError(
