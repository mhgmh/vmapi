...
func (s *BillingService) getDefaultVideoPrice(model string, resolution string) float64 {
	...
	if price, ok := getDefaultSeedanceVideoPrice(model); ok {
		return price
	}
	...
	// rate; today Grok/Seedance models reach video billing, so this path is a safety net),
...
// seedanceCNYToUSD is a rough conversion used only for catalog defaults when group
// video prices are not configured. Operators should set group-level USD prices in production.
const seedanceCNYToUSD = 0.14

func getDefaultSeedanceVideoPrice(model string) (float64, bool) {
	spec, ok := LookupSeedanceModel(strings.TrimSpace(model))
	if !ok || spec.UnitPriceCNY <= 0 {
		return 0, false
	}
	// Per-request models: CalculateVideoCost multiplies by durationSeconds.
	// Return unit price as full request price / 1s so duration=1 yields fixed price.
	// Callers for per-request Seedance models should pass durationSeconds=1.
	return spec.UnitPriceCNY * seedanceCNYToUSD, true
}
