...
func TestFetchUpstreamSupportedModelsParsesSeedanceAPIKeyResponse(t *testing.T) {
	t.Parallel()

	upstream := &httpUpstreamRecorder{resp: &http.Response{
		StatusCode: http.StatusOK,
		Header:     http.Header{"Content-Type": []string{"application/json"}},
		Body:       io.NopCloser(strings.NewReader(`{"data":[{"id":"bh2.0-720p"},{"id":"SD2.0-720P"},{"id":"bh2.0-720p"}]}`)),
	}}
	svc := &AccountTestService{
		httpUpstream: upstream,
		cfg:          upstreamModelSyncTestConfig(),
	...
	require.NoError(t, err)
	require.Equal(t, []string{"SD2.0-720P", "bh2.0-720p"}, models)
	require.Equal(t, "https://api.7tai.cc/v1/models", upstream.lastReq.URL.String())
	require.Equal(t, "Bearer sk-seedance", upstream.lastReq.Header.Get("Authorization"))
}

func TestBuildSeedanceUpstreamModelsRequestRejectsNonAPIKey(t *testing.T) {
	t.Parallel()
