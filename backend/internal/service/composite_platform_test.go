...
func TestDetectModelPlatform(t *testing.T) {
	tests := []struct {
		...
		{name: "seedance catalog", model: "bh2.0-720p", platform: PlatformSeedance, ok: true},
		{name: "seedance case sensitive id", model: "SD2.0-720P", platform: PlatformSeedance, ok: true},
		{name: "seedance provider prefix", model: "seedance/bh2.0-720p", platform: PlatformSeedance, ok: true},
...
func TestCompositeGroupSchedulerHasAllCanonicalPlatformBuckets(t *testing.T) {
	...
	require.ElementsMatch(t,
		[]string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformGrok, PlatformSeedance},
		platforms,
	)
}
