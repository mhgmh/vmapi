...
func defaultAllowImageGenerationForPlatform(platform string) bool {
	// Grok image/video and Seedance video routes share the legacy image-generation gate.
	// Older clients send the false zero value, so these media groups must default enabled.
...
func compositeDefaultModelsListCandidateIDs() []string {
	...
	ids := make([]string, 0)
	for _, platform := range []string{PlatformAnthropic, PlatformGemini, PlatformOpenAI, PlatformAntigravity, PlatformGrok, PlatformSeedance} {
