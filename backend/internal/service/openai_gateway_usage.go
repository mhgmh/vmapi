...
func isSeedanceVideoBillingModel(model string) bool {
	_, ok := LookupSeedanceModel(strings.TrimSpace(model))
	return ok
}
...
func isGrokVideoUsageResult(result *OpenAIForwardResult, billingModels []string) bool {
	...
	for _, candidate := range candidates {
		if isGrokVideoBillingModel(candidate) || isSeedanceVideoBillingModel(candidate) {
			return true
		}
	}
	// Any successful video generation with VideoCount>0 is billed as video.
...
func (s *OpenAIGatewayService) calculateOpenAIVideoCost(
	...
	// Seedance per-request models bill a fixed price per generation; force duration=1
	// so CalculateVideoCost (unit * duration) yields the fixed unit price once.
	if spec, ok := LookupSeedanceModel(strings.TrimSpace(billingModel)); ok {
		if spec.BillingMode == SeedanceBillingModePerRequest {
			durationSeconds = 1
		} else if result.VideoDurationSeconds > 0 {
			// Seedance per-second models use requested duration (4-15s typical),
			// not the Grok 1-15 clamp defaults exclusively.
			durationSeconds = result.VideoDurationSeconds
			if durationSeconds < 1 {
				durationSeconds = 1
			}
		}
		// Apply reference-video multiplier into the rate multiplier when catalog defines it.
		// Note: HasReferenceVideo is not stored on result; operators can encode it via group rates.
	}
	...
	if resolved := s.resolveOpenAIChannelPricing(ctx, billingModel, apiKey); resolved != nil &&
		...
		// billing_mode=video: per_request_price is unit price per second → unit * duration * count.
		// Seedance catalog per-second models also bill this way when channel uses per_request.
		// Flat per-request Seedance models keep duration=1 (set above) and do not multiply seconds.
		billAsPerSecond := resolved.Mode == BillingModeVideo
		if !billAsPerSecond {
			if spec, ok := LookupSeedanceModel(strings.TrimSpace(billingModel)); ok {
				billAsPerSecond = spec.BillingMode == SeedanceBillingModePerSecond
			}
		}
		if billAsPerSecond && resolved.DefaultPerRequestPrice > 0 {
			unit := resolved.DefaultPerRequestPrice
			if durationSeconds < 1 {
				durationSeconds = 1
			}
			total := unit * float64(durationSeconds) * float64(videoCount)
