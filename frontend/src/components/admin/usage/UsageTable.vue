      ...
      >
        <template #cell-user="{ row }">
          <div class="text-sm">
            ...
            <span v-else class="font-medium text-gray-900 dark:text-dark-50">-</span>
        ...
        <template #cell-api_key="{ row }">
          <span class="text-sm text-gray-900 dark:text-dark-50">{{ row.api_key?.name || '-' }}</span>
        </template>

        <template #cell-account="{ row }">
          <span class="text-sm text-gray-900 dark:text-dark-50">{{ row.account?.name || '-' }}</span>
        </template>
        ...
        <template #cell-model="{ row }">
          <div v-if="row.model_mapping_chain && row.model_mapping_chain.includes('→')" class="space-y-0.5 text-xs">
            <div v-for="(step, i) in row.model_mapping_chain.split('→')" :key="i"
                 class="break-all"
                 :class="i === 0 ? 'font-medium text-gray-900 dark:text-dark-50' : 'text-gray-500 dark:text-gray-400'"
          ...
          <div v-else-if="row.upstream_model && row.upstream_model !== row.model" class="space-y-0.5 text-xs">
            <div class="break-all font-medium text-gray-900 dark:text-dark-50">
              {{ row.model }}
            </div>
          ...
          <span v-else class="font-medium text-gray-900 dark:text-dark-50">{{ row.model }}</span>
        </template>

        <template #cell-reasoning_effort="{ row }">
          <span class="text-sm text-gray-900 dark:text-dark-50">
        ...
        <template #cell-tokens="{ row }">
          ...
          <div v-if="isImageUsage(row)" class="flex items-center gap-1.5">
            ...
            <span class="font-medium text-gray-900 dark:text-dark-50">{{ row.image_count }}{{ t('usage.imageUnit') }}</span>
            ...
            <div class="space-y-1 text-sm">
              <div class="flex items-center gap-2">
                <div class="inline-flex items-center gap-1">
                  ...
                  <span class="font-medium text-gray-900 dark:text-dark-50">{{ row.input_tokens?.toLocaleString() || 0 }}</span>
                </div>
                <div class="inline-flex items-center gap-1">
                  <Icon name="arrowUp" size="sm" class="h-3.5 w-3.5 text-violet-500" />
                  <span class="font-medium text-gray-900 dark:text-dark-50">{{ row.output_tokens?.toLocaleString() || 0 }}</span>
                </div>
              </div>
