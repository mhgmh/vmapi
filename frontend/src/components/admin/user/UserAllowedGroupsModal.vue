<template>
  <BaseDialog :show="show" :title="t('admin.users.groupConfig')" width="wide" @close="$emit('close')">
    <div v-if="user" class="space-y-6">
      <!-- 用户信息头部 -->
      <div class="flex items-center gap-4 rounded-lg border border-primary-200 bg-primary-50 p-5 dark:border-primary-400/20 dark:bg-primary-400/10">
        ...
        <div class="flex-1">
          <p class="text-lg font-semibold text-gray-900 dark:text-dark-50">{{ user.email }}</p>
              ...
              <div class="flex items-center gap-4">
                ...
                <div class="min-w-0 flex-1">
                  <div class="flex items-center gap-2">
                    <span class="text-base font-semibold text-gray-900 dark:text-dark-50">{{ config.groupName }}</span>
