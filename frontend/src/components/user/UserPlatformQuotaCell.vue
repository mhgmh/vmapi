  ...
  <div v-else class="space-y-0.5 text-xs">
    ...
    >
      ...
      <span class="text-gray-500 dark:text-gray-400">
        ...
        <span class="text-gray-900 dark:text-dark-50">{{ fmtUsd(row.daily_usage_usd) }}/{{ fmtLimit(row.daily_limit_usd) }}</span>
      </span>
      <span class="text-gray-500 dark:text-gray-400">
        {{ t('admin.users.platformQuota.windowWeekly') }}
        <span class="text-gray-900 dark:text-dark-50">{{ fmtUsd(row.weekly_usage_usd) }}/{{ fmtLimit(row.weekly_limit_usd) }}</span>
      </span>
      <span class="text-gray-500 dark:text-gray-400">
        {{ t('admin.users.platformQuota.windowMonthly') }}
        <span class="text-gray-900 dark:text-dark-50">{{ fmtUsd(row.monthly_usage_usd) }}/{{ fmtLimit(row.monthly_limit_usd) }}</span>
      </span>
    </div>
  </div>
</template>
