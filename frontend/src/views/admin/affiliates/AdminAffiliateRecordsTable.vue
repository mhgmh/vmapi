        ...
        >
          ...
          <template #cell-order="{ row }">
            <div class="space-y-0.5">
              <div class="font-mono text-sm text-gray-900 dark:text-dark-50">#{{ row.order_id }}</div>
          ...
          <template #cell-pay_amount="{ row }">
            <span class="text-sm text-gray-900 dark:text-dark-50">¥{{ formatAmount(row.pay_amount) }}</span>
          </template>
    ...
    >
      ...
      <div v-else-if="selectedOverview" class="space-y-4">
        <div class="rounded-lg border border-gray-100 bg-gray-50 p-4 dark:border-dark-700 dark:bg-dark-800">
          <div class="font-mono text-sm text-gray-900 dark:text-dark-50">#{{ selectedOverview.user_id }}</div>
          <div class="mt-1 text-sm font-medium text-gray-900 dark:text-dark-50">{{ selectedOverview.email || '-' }}</div>
...
const UserCell = defineComponent({
  ...
  setup(cellProps, { emit }) {
    return () => h('div', { class: 'space-y-0.5' }, [
      h('div', { class: 'font-mono text-sm text-gray-900 dark:text-dark-50' }, `#${cellProps.id}`),
  ...
  setup(amountProps) {
    return () => h('span', {
      class: amountProps.strong
        ...
        : 'text-sm text-gray-900 dark:text-dark-50',
    ...
    return () => h('div', { class: 'rounded-lg border border-gray-100 bg-white p-3 dark:border-dark-700 dark:bg-dark-900' }, [
      ...
      h('div', {
        class: statProps.mono
          ? 'mt-1 font-mono text-base font-semibold text-gray-900 dark:text-dark-50'
          : 'mt-1 text-base font-semibold text-gray-900 dark:text-dark-50',
      }, statProps.value),
    ])
  },
})
