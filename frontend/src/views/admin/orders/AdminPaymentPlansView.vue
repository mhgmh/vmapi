    ...
    <div class="space-y-4">
      ...
      <DataTable :columns="planColumns" :data="plans" :loading="plansLoading">
        ...
        <template #cell-price="{ value, row }">
          <div class="text-sm">
            <span class="font-medium text-gray-900 dark:text-dark-50">{{ planCurrencySymbol(row.currency) }}{{ (value ?? 0).toFixed(2) }}</span>
...
function getPlanNameClass(groupId: number): string {
  ...
  return group ? platformTextClass(group.platform) : 'text-gray-900 dark:text-dark-50'
}
