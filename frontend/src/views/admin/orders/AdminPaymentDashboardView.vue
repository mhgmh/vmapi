    ...
    <div class="space-y-6">
      ...
      <template v-else-if="stats">
        ...
        <div class="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <div class="card p-4">
            <h3 class="mb-4 text-sm font-semibold text-gray-900 dark:text-dark-50">{{ t('payment.admin.paymentDistribution') }}</h3>
            ...
            <div v-else class="space-y-3">
              <div v-for="method in stats.payment_methods" :key="method.type" class="flex items-center justify-between">
                ...
                <div class="text-right">
                  <span class="text-sm font-medium text-gray-900 dark:text-dark-50">&yen;{{ method.amount.toFixed(2) }}</span>
          ...
          <div class="card p-4">
            <h3 class="mb-4 text-sm font-semibold text-gray-900 dark:text-dark-50">{{ t('payment.admin.topUsers') }}</h3>
            ...
            <div v-else class="space-y-2">
              <div v-for="(user, idx) in stats.top_users" :key="user.user_id" class="flex items-center justify-between rounded-lg px-3 py-2 hover:bg-gray-50 dark:hover:bg-dark-700">
                ...
                <span class="text-sm font-medium text-gray-900 dark:text-dark-50">&yen;{{ user.amount.toFixed(2) }}</span>
              </div>
            </div>
          </div>
        </div>
