      ...
      <template v-else>
        ...
        <div v-if="tabs.length > 1 && paymentPhase === 'select' && !selectedPlan" class="flex space-x-1 rounded-xl bg-gray-100 p-1 dark:bg-dark-800">
          <button v-for="tab in tabs" :key="tab.key"
            ...
            :class="activeTab === tab.key ? 'bg-white text-gray-900 shadow dark:bg-dark-700 dark:text-dark-50' : 'text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-300'"
        ...
        <template v-else>
          ...
          <template v-if="activeTab === 'recharge'">
            ...
            <div class="card p-5">
              ...
              <p class="mt-1 text-base font-semibold text-gray-900 dark:text-dark-50">{{ user?.username || '' }}</p>
            ...
            <div v-if="validAmount > 0" class="card p-6">
              <div class="space-y-2 text-sm">
                <div class="flex justify-between">
                  ...
                  <span class="text-gray-900 dark:text-dark-50">{{ formatSelectedPaymentAmount(validAmount) }}</span>
                </div>
                <div v-if="feeRate > 0" class="flex justify-between">
                  <span class="text-gray-500 dark:text-gray-400">{{ t('payment.fee') }} ({{ feeRate }}%)</span>
                  <span class="text-gray-900 dark:text-dark-50">{{ formatSelectedPaymentAmount(feeAmount) }}</span>
                </div>
                ...
                <div v-if="balanceRechargeMultiplier !== 1" class="flex justify-between" :class="{ 'border-t border-gray-200 pt-2 dark:border-dark-600': feeRate <= 0 }">
                  ...
                  <span class="text-gray-900 dark:text-dark-50">${{ creditedAmount.toFixed(2) }}</span>
                </div>
            ...
            <template v-if="selectedPlan">
              <div class="card p-5">
                ...
                <div class="mb-3 flex flex-wrap items-center gap-2">
                  ...
                  <h3 class="text-lg font-bold text-gray-900 dark:text-dark-50">{{ selectedPlan.name }}</h3>
                </div>
                <!-- Price -->
              ...
              <div v-if="feeRate > 0 && selectedPlan.price > 0" class="card p-6">
                <div class="space-y-2 text-sm">
                  <div class="flex justify-between">
                    ...
                    <span class="text-gray-900 dark:text-dark-50">{{ formatSelectedPaymentAmount(subPaymentAmount) }}</span>
                  </div>
                  <div class="flex justify-between">
                    <span class="text-gray-500 dark:text-gray-400">{{ t('payment.fee') }} ({{ feeRate }}%)</span>
                    <span class="text-gray-900 dark:text-dark-50">{{ formatSelectedPaymentAmount(subFeeAmount) }}</span>
                  </div>
                  ...
                  <div v-for="sub in activeSubscriptions" :key="sub.id"
                    ...
                    <div class="min-w-0 flex-1">
                      <div class="flex items-center gap-1.5">
                        <span class="truncate text-xs font-semibold text-gray-900 dark:text-dark-50">{{ sub.group?.name || t('payment.groupFallback', { id: sub.group_id }) }}</span>
      ...
      <Transition name="modal">
        <div v-if="showRenewalModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" @click.self="closeRenewalModal">
          <div class="relative w-full max-w-lg rounded-lg border border-gray-200 bg-white p-6 shadow-2xl dark:border-dark-700 dark:bg-dark-900">
            ...
            <h3 class="mb-4 text-lg font-semibold text-gray-900 dark:text-dark-50">{{ t('payment.selectPlan') }}</h3>
            <div class="space-y-4">
